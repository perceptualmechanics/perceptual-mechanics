...
function seance({ seed, minutes }) {
  ...
  for (let i = 0; i < Math.round((minutes * 60) / DT); i++) {
    ...
    stepCup(cup, DT, stepVisitor(visitor, cup, DT), stepWander(hand, DT, cup, MARKS, plaus, visitor.grip));
...
<p class="note">It will not say any of this to you. The other hand’s wander is seeded and these three seeds are fixed so that the page is the same page twice; the scene takes a new seed every visit and writes it into the address bar, so a séance is linkable while you are in it. What a link brings back is the other hand — the same rhythm, the same impulses, the same starting place — and not the message. The same seed with a hand resting on the cup, with a hand that nudges it, and with nobody touching it produce three different tapes, the last of them empty. That is not a limitation of the link. There is no message anywhere in the code to carry.</p>
