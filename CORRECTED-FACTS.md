...
| Apollo's preview cannot get a zero-sized band, because W and H are clamped to a minimum of 1 | **`bandH` was not clamped, and a third of 1 rounds to zero.** Apollo is the only scene that hands a computed height to `createImageData`, which throws on a zero — and because that rejects `create()`, the tile stays blank for the entire visit rather than recovering when layout settles. Seen once as `preview "apollo" did not load — IndexSizeError`, in the desktop app's browser pane, and **not reproduced** at any viewport down to 1px tall, with a zero-height container, or with a hidden one. Guarded at 4.8.6; the guard is certain and the diagnosis is not. | `src/scenes/apollo/apollo.js`, `layout()` preview branch |
