# NOTES.archive.md — the version history through 4.8.9

**This file is history. Nothing in it is a statement about the site as it
stands.**

It is the version-by-version changelog from 1.0 (2026-07-17) through 4.8.9,
moved out of `NOTES.md` at 5.0 because that file had reached 17,500 lines and
the standing rules at its top — the part anyone actually needs to read before
working — were buried under sixteen thousand lines of dated entries.

`NOTES.md` keeps the standing sections, the current state, and every release
from 4.9.0 on. When the working file gets long again, the next block moves here
and this header says so.

**Read this file for one thing only: why a decision was made.** A dated entry is
the only record of an alternative that was tried and rejected, and re-proposing
one of those is the specific waste this history exists to prevent. Do NOT read
it for a fact about the site — every count, every file path and every claim in
here was true on its own date and many of them are not true now. `SITE.md` is
what the site is; `STANDARDS.md` is how we build it; `CORRECTED-FACTS.md` is the
list of claims that were wrong and came back.

The build's `verify-counts` check deliberately does not read this file, for
exactly that reason.
