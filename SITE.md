...
**3,268 sentences, 35,089 words, in 159 pieces across 10 scenes.** The narrower
...
**Do not quote the paragraph above; run `node scripts/corpus-figures.mjs`.**
It prints all four numbers and the per-scene breakdown from the corpus itself.
These figures were measured by hand once and typed here, and on 2026-09-06 a
fourteenth scene made every one of them wrong in the same instant, in the
document a session is supposed to read *before* it starts counting. The
paragraph is kept because a reader wants a magnitude in the sentence they are
reading; the command is what makes it checkable.

**The wide figure has NOT been recomputed since Quiz.** The bench does the
narrow ruler only, because the wide one — scene slugs, stage directions, the
Library's bibliographic entries — was a hand-count whose rules were never
written as code. Quiz adds 283 words under the narrow ruler. Treat 36,886 as
*at least* that, and as of 4.11.
...
**Three exempt with a stated reason, and that is a fact about them rather
than an omission.** Harmonics is a view of the connections between other
